<script lang="ts">
	import { page } from '$app/state';

	const error = $derived(page.url.searchParams.get('error'));

	const errorMessages: Record<string, string> = {
		not_member: 'You must be a member of the infragnite organization.',
		auth_failed: 'Authentication failed. Please try again.',
		invalid_state: 'Session expired. Please try again.',
		no_code: 'Authentication was cancelled.'
	};

	const errorMessage = $derived(error ? errorMessages[error] || 'Something went wrong. Please try again.' : null);
</script>

<svelte:head>
	<title>Sign In - Merge Queue</title>
</svelte:head>

<div class="flex min-h-screen items-center justify-center px-4">
	<div class="w-full max-w-sm">
		<div class="rounded-xl border border-border-subtle bg-surface-raised p-8 shadow-lg">
			<div class="mb-6 text-center">
				<div class="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-xl bg-indigo-600">
					<svg class="h-7 w-7 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
						<path stroke-linecap="round" stroke-linejoin="round" d="M4 6h16M4 12h12M4 18h8" />
					</svg>
				</div>
				<h1 class="text-2xl font-bold text-gray-900">Merge Queue</h1>
				<p class="mt-2 text-sm text-gray-500">
					A simple merge queue for your GitHub repositories
				</p>
			</div>

			{#if errorMessage}
				<div class="mb-4 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700">
					{errorMessage}
				</div>
			{/if}

			<a
				href="/auth/github"
				class="flex w-full items-center justify-center gap-3 rounded-lg bg-gray-900 px-4 py-3 text-sm font-medium text-white transition-colors hover:bg-gray-800"
			>
				<svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
					<path
						d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"
					/>
				</svg>
				Sign in with GitHub
			</a>
		</div>

		<p class="mt-6 text-center text-xs text-gray-400">
			Restricted to infragnite organization members
		</p>
	</div>
</div>
