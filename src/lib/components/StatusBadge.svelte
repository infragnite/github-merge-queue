<script lang="ts">
	...
	const config = $derived.by(() => {
		switch (status) {
			...
			case 'conflict':
				return { label: 'Conflict', class: 'bg-orange-100 text-orange-700' };
			case 'failed':
	...
	const isActive = $derived(['updating', 'checking', 'merging', 'conflict'].includes(status));
</script>

<span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium {config.class}">
	{#if isActive}
		<span class="relative flex h-2 w-2">
			<span class="absolute inline-flex h-full w-full animate-ping rounded-full opacity-75 {status === 'updating' ? 'bg-blue-400' : status === 'checking' ? 'bg-amber-400' : status === 'conflict' ? 'bg-orange-400' : 'bg-purple-400'}"></span>
			<span class="relative inline-flex h-2 w-2 rounded-full {status === 'updating' ? 'bg-blue-500' : status === 'checking' ? 'bg-amber-500' : status === 'conflict' ? 'bg-orange-500' : 'bg-purple-500'}"></span>
		</span>
	{/if}
	{config.label}
</span>
